...
export const routes: IRoute[] = [
  {
    ...
    children: [
      {
        name: 'menu.dashboard.assetManagement',
        key: 'dashboard/asset-management',
      },
      {
        name: 'menu.dashboard.toolComponent',
        key: 'dashboard/tool-component',
      },
      {
      ...
      {
        name: 'menu.dashboard.agentFlow',
        key: 'dashboard/agent-flow',
      },
      {
...
const useRoute = (userPermission): [IRoute[], string] => {
  // 确保 userPermission 是一个对象
  const safeUserPermission = userPermission || {};
  
  const filterRoute = (routes: IRoute[], arr: IRoute[] = [], userPerm = safeUserPermission): IRoute[] => {
    if (!routes.length) {
      return [];
    }
    for (const route of routes) {
      ...
      if (requiredPermissions) {
        visible = auth({ requiredPermissions, oneOfPerm }, userPerm);
      }
      ...
      if (route.children && route.children.length) {
        ...
        filterRoute(route.children, newRoute.children, userPerm);
